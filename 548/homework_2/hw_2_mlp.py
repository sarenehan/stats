...
def train_mlp_nesterov(
    ...
    val_average_precision_scores = []
    iteration_num = 0
    while train_error > 0.01 and iteration_num < 35000:
        if not iteration_num % half_epoch:
            train_error = float(compute_error(w1, w2, x_data, y_data))
            val_error = float(compute_error(w1, w2, x_val, y_val))
            train_average_precision_score = get_average_precision_score(
...
def train_mlp_adagrad(
    ...
    epsilon_1 = torch.zeros(w1.shape) + (0.00000001)
    epsilon_2 = torch.zeros(w2.shape) + (0.00000001)
    indexes = list(range(x_data.size()[0]))
    train_error = float('inf')
    half_epoch = int(x_data.size()[0] / (2 * mini_batch_size))
    print('Half ephoc: {}'.format(half_epoch))
    ...
    train_average_precision_scores = []
    val_average_precision_scores = []
    grad_sizes_1 = torch.zeros(w1.shape)
    grad_sizes_2 = torch.zeros(w2.shape)
    iteration_num = 0
    while train_error > 0.01 and iteration_num < 35000:
        if not iteration_num % half_epoch:
            train_error = float(compute_error(w1, w2, x_data, y_data))
        ...
        iteration_num += 1
        idx = random.sample(indexes, mini_batch_size)
        err_est = compute_error(w1, w2, x_data[idx], y_data[idx])
        err_est.backward()

        grad_sizes_1 = grad_sizes_1 + w1.grad.pow(2).data
        grad_sizes_2 = grad_sizes_2 + w2.grad.pow(2).data
        step_size_1 = training_rate / torch.sqrt(grad_sizes_1 + epsilon_1)
        step_size_2 = training_rate / torch.sqrt(grad_sizes_2 + epsilon_2)
        w1.data = w1.data - (step_size_1 * w1.grad.data)
        w2.data = w2.data - (step_size_2 * w2.grad.data)
        w1.grad.data.zero_()
        w2.grad.data.zero_()
    return (
        train_errors,
...
def get_mlp_adagrad():
    ...
    (
        ...
        w1,
        w2
    ) = train_mlp_adagrad(training_rate, mini_batch_size=mini_batch_size)
    with open(save_dir + '{}_mlp_weights_adagrad.pkl'.format(
            n_hidden_nodes), 'wb') as f:
        pickle.dump((w1, w2), f)
    with open(save_dir + '{}_mlp_errors_adagrad.pkl'.format(
            n_hidden_nodes), 'wb') as f:
        pickle.dump((
            train_errors,
            val_errors,
